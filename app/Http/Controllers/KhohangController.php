...
class KhohangController extends Controller
{
    ...
    public function getdanhsach_kho(){
        $khohang=khohang::all();
    	return view('quanly.kho.khohang.danhsach_kho')->with('khohang',$khohang);
    }
    public function gettao_kho(){
      
    	return view('quanly.kho.khohang.tao_kho');
    }
     ...
     public function posttao_kho(Request $request){
      ...
      return redirect('banhang/danhsach-kho')->with('thongbao','Tạo kho không thành công');
}
  ...
  public function getsua_kho($kho_id){
       $khohang=khohang::find($kho_id);
    	return view('quanly.kho.khohang.sua_kho',['khohang'=>$khohang]);
    }
     public function postsua_kho(Request $request,$kho_id){
      ...
      return redirect('banhang/danhsach-kho')->with('thongbao','Tên kho đã tồn tại');
}
  ...
  try {
          $khohang->delete();
        return redirect('banhang/danhsach-kho')->with('thongbao','Bạn đã xóa kho thành công');
  
} catch (\Exception $e)  {
     return redirect('banhang/danhsach-kho')->with('thongbao','Bạn đã xóa kho không thành công (do kho có ràng buộc dữ liệu)');
}
    }
}
